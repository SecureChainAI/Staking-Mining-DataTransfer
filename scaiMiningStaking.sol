...
contract scaiMiningStaking is Ownable,  ReentrancyGuard {
    ...
    mapping(uint256 => UserStake[]) public stakeInfo;

    mapping(address => uint256) public getUserIdForAddress;
    ...
    // user withdraws staked coins 
    function WithdrawStake(uint256  arrayIndex) external nonReentrant returns (bool)
    {
        uint256 accountId = getUserIdForAddress[msg.sender];

        // Stake should exists and opened
        require( stakeInfo[accountId].length >arrayIndex , "Stake does not exist");
        UserStake storage stakedata = stakeInfo[accountId][arrayIndex];
        ...
        emit Withdraw(msg.sender, arrayIndex, unstakeAmount, block.timestamp);
        return true;
    }

    // Updates stake data
    function editStake(uint256 inx, uint256 _accountId, uint256 amt,LockingPeriod lockduration,  uint256 starttime,uint256 endtime) external onlyOwner
    {   

        UserStake storage stakedata = stakeInfo[_accountId][inx];
        ...
        emit StakeUpdated(msg.sender, amt, inx);
    }
    ...
    // owner can rescue stucked coins from contract
    function rescueCoins() external onlyOwner {
        payable(msg.sender).transfer(address(this).balance);
    }


    // loads database data to contract
    ...
    uint256[] memory stakeEndTimeArray) public onlyManager
    {
        for (uint i=0; i<accountIdArray.length; i++) { // unbounded loop DoS warning
             uint256 accountId = accountIdArray[i];
            ...
            stakeInfo[accountId].push(newUser);
            totalStakeAmount = totalStakeAmount + stakingAmount[i];
            totalStakers += 1;
        }
    }
    
    // sets account ID for users
    function SetAccountIdForAddress( uint256[] memory accountIdArray,address[] memory userAddress ) public onlyManager
    {   
        for (uint i=0; i<accountIdArray.length; i++) {
            address _user = userAddress[i];
            getUserIdForAddress[_user] = accountIdArray[i];
        }
    }


    // Function to get all stakes of a user
    function getUserStakes(address user) public view returns (UserStake[] memory) {
        uint256 accountId = getUserIdForAddress[user];
        return stakeInfo[accountId];
    }

    /**
     * @dev Returns the total number of one user's stakes.
     */
    function getUserStakeLength(address user) public view returns (uint256) {
        uint256 accountId = getUserIdForAddress[user];
        return stakeInfo[accountId].length;
    }
